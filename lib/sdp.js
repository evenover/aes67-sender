var dgram = require('dgram');
var socket = dgram.createSocket({ type: 'udp4', reuseAddr: true });

let hash = Math.floor(Math.random() * 65536);

var constructSDPMsg = function(addr, multicastAddr, samplerate, channels, encoding, name, sessID, sessVersion, ptpMaster, ptpDomain){
	var sapHeader = Buffer.alloc(8);
	var sapContentType = Buffer.from('application/sdp\0');
	var ip = addr.split('.');

	//write version/options
	sapHeader.writeUInt8(0x20);

	//write hash
	sapHeader.writeUInt16LE(hash, 2);

	//write ip
	sapHeader.writeUInt8(parseInt(ip[0]), 4);
	sapHeader.writeUInt8(parseInt(ip[1]), 5);
	sapHeader.writeUInt8(parseInt(ip[2]), 6);
	sapHeader.writeUInt8(parseInt(ip[3]), 7);

	// ptpMaster is "XX-XX-XX-FF-FE-XX-XX-XX:0" — split off the domain suffix
	// RFC 7273 ABNF requires uppercase HEXDIG for EUI-64
	var ptpEUI = ptpMaster.replace(/:\d+$/, '').toUpperCase();

	var sdpConfig = [
		'v=0',
		'o=- '+sessID+' '+sessVersion+' IN IP4 '+addr,
		's='+name,
		'c=IN IP4 '+multicastAddr+'/32',
		't=0 0',
		'a=clock-domain:PTPv2 '+ptpDomain,
		'm=audio 5004 RTP/AVP 96',
		'a=rtpmap:96 '+encoding+'/'+samplerate+'/'+channels,
		'a=sync-time:0',
		'a=framecount:48',
		'a=ptime:1',
		'a=mediaclk:direct=0',
		'a=ts-refclk:ptp=IEEE1588-2008:'+ptpEUI+':'+ptpDomain,
		'a=source-filter: incl IN IP4 '+multicastAddr+' '+addr,
		'a=sendonly',
		''
	];
	console.log('Constructed SDP:\n' + sdpConfig.join('\n'));
	var sdpBody = Buffer.from(sdpConfig.join('\r\n'));

	return Buffer.concat([sapHeader, sapContentType, sdpBody]);
}

exports.start = function(addr, multicastAddr, samplerate, channels, encoding, name, sessID, sessVersion, ptpMaster, ptpDomain){
	sdpMSG = constructSDPMsg(addr, multicastAddr, samplerate, channels, encoding, name, sessID, sessVersion, ptpMaster, ptpDomain);

	socket.bind(9875, addr, function(){
		socket.setMulticastInterface(addr);
		socket.send(sdpMSG, 9875, '239.255.255.255', function(err){});
	});
	
	setInterval(function(){
		socket.send(sdpMSG, 9875, '239.255.255.255', function(err){});
	}, 30*1000);
}
